...
public class Main {
    ...
    public static final String TESTINSTANCE = "Instance Input Files\\testing instance.txt";
//    private static final String FILENAME = "Instance Input Files\\smallish instance.txt";
    ...
    private static final int NUMSTUDENTS = (useTrueNames ? 21945 : 69);
    private static final int MAXCOURSESPERSTUDENT = 6;
    private static final int NUMROOMS = (useTrueNames ? trueRoomNames.length : 8);
    private static final int NUMCOURSECODES = 5;
    // It is good to have this a multiple of NUMCOURSECODES
    private static final int NUMCOURSES = (useTrueNames ? trueCourseNames.length : 100);
    private static final int NUMCOURSESPERCODE = NUMCOURSES / NUMCOURSECODES;

    public static void main(String[] args) {
        System.out.println("Generated set properties:");
        System.out.println(NUMDAYS + " days, " + NUMTIMES + " times");
        System.out.println(NUMSTUDENTS + " students");
        System.out.println(NUMCOURSES + " courses");
        System.out.println(NUMROOMS + " rooms");
        System.out.println("Outfile is \"" + FILENAME + "\"");
...
//            writeStream out = System.out;
    ...
    private static void generateStudent(Random rand, BufferedWriter out) throws IOException {
        double probOfBeingInMyCoursecode = 0.7;
        // Students take at least 2 courses, at most MAXCOURSESPERSTUDENT
        int numCourses = 2 + rand.nextInt(MAXCOURSESPERSTUDENT - 1);
        if (numCourses == 0) {
            numCourses = 1;
        }
        // Choose the student's course code randomly
        // We don't weight then in any way (eg. same number of english majors as CS majors)
        int myCode = rand.nextInt(NUMCOURSECODES);

        out.write(numCourses + "\n");
        Set selectedCourses = new TreeSet();
        for (int k = 0; k < numCourses; k++) {
            int coursenum = 0;

            if (rand.nextDouble() < probOfBeingInMyCoursecode) {
                // Guaranteed to be in the range chosen for mycoursecode
                coursenum = NUMCOURSESPERCODE * myCode + rand.nextInt(NUMCOURSESPERCODE);
            } else {
                // Select among all courses
                // The 1 is because we name them Course 1... Course n
                coursenum = 1 + rand.nextInt(NUMCOURSES);
            }

            // Try again if we're taking the selected course already
            if (selectedCourses.contains(coursenum)) {
                k--;
            } else {
